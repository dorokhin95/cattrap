...
## 6. Принцип визуализации ловушек (Zero-Hint Trap Architecture)

Все скрытые опасности реализуют принцип нулевого визуального телеграфирования:
1. **Текстурный изоморфизм:**
   - `tile_fake` генерируется идентично `tile_solid` (отсутствуют искусственные смещения швов);
   - `pop_spike_floor` в закрытом виде является точной копией `tile_solid` (отсутствуют темные пиксели щели);
   - `falling_block` монолитен по текстуре (отсутствуют оранжевые warning-пиксели).
2. **Динамическая смена текстур по событию:**
   - `CrumbleBlock` спаунится с текстурой `tile_solid`. Переключение на текстуру `tile_crumble` с трещиной происходит строго при вызове `triggerCollapse()` после касания котиком. При сбросе (`reset()`) возвращается `tile_solid`.
3. **Невидимость зон управления:**
   - `ControlZone` (инверсия `reverse` и автобег `autorun_right`) скрыта до контакта (`visible = false`). Индикация и звуковое сопровождение активируются только в момент входа котика на короткий период.
