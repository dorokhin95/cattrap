...
│   ├── game/
│   │   ├── BackgroundRenderer.ts   # Двухслойный бесшовный параллакс заднего плана (Глава 1 и 2)
│   │   ├── CameraSystem.ts         # Адаптивная камера (portrait/landscape, look-ahead)
│   │   ├── entities/
│   │   │   └── Cat.ts              # Сущность котика: физика, coyote time, jump buffer, модификаторы управления
│   │   ├── hazards/
│   │   │   ├── BouncePad.ts        # Батут-пружина (Глава 2)
│   │   │   ├── Checkpoint.ts       # Коробка-сохранение спауна (Главы 1 и 2)
│   │   │   ├── ControlZone.ts      # Зона изменения управления (reverse, autorun_right, normal)
│   │   │   ├── ConveyorTile.ts     # Конвейерная плита (Глава 2)
│   │   │   ├── CrumbleBlock.ts     # Осыпающиеся блоки
│   │   │   ├── Crusher.ts          # Тяжёлый гидравлический пресс (Глава 2)
...
│   │   │   ├── MovingPlatform.ts   # Движущаяся платформа (Глава 2)
│   │   │   ├── MovingPortal.ts     # Убегающий портал
│   │   │   ├── PopSpike.ts         # Выдвижной шип в полу
│   │   │   ├── PressureButton.ts   # Нажимная кнопка переключения (Глава 2)
│   │   │   ├── StaticSpike.ts      # Статический шип с честным хитбоксом
│   │   │   └── ToggleBlock.ts      # Переключаемый твердый блок (Глава 2)
│   │   ├── levels/
│   │   │   ├── LevelData.ts        # Типы и структуры данных уровней (Главы 1 и 2)
│   │   │   ├── LevelRegistry.ts    # Реестр и загрузчик 20 уровней
│   │   │   ├── level01.ts - level10.ts # Уровни 1–10: Глава 1 («Ничему не верь»)
│   │   │   └── level11.ts - level20.ts # Уровни 11–20: Глава 2 («Не верь правилам движения»)
│   │   └── triggers/
...
│   │   ├── LevelSelectScene.ts     # Экран выбора уровней (Глава 1 и Глава 2, статусы, замки, рекорды)
│   │   ├── MenuScene.ts            # Главное меню (Играть/Продолжить, Уровни, Настройки, анимированный кот)
│   │   └── UIScene.ts              # HUD, тач-кнопки, экраны паузы, настроек, триумфа Глав 1 и 2
│   ├── types/
...
├── tests/
│   ├── audit-fixes.test.ts         # Тесты исправлений утечек, санитизации и регрессий
│   ├── cat-physics.test.ts         # Тесты физики (coyote time, jump buffer, переменный прыжок)
│   ├── chapter2.test.ts            # Комплексные тесты Главы 2 (механики, реестр, контракты, сброс)
│   ├── hazards.test.ts             # Тесты хитбоксов и логики ловушек
│   ├── levels.test.ts              # Тесты валидности структуры всех 20 уровней
