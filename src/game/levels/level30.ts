...
export const level30: LevelData = {
  ...
  solidTiles: [
    ...
    // Верхний постамент для переместившегося портала (x=57..60, y=8..9) — 2 тайла над полом
    ...Array.from({ length: 4 }, (_, i) => ({ x: 57 + i, y: 8 })),
    ...Array.from({ length: 4 }, (_, i) => ({ x: 57 + i, y: 9 })),
  ...
  // Батут на правом крае реверс-секции и на финишной платформе — чтобы запрыгнуть на постамент с порталом
  bouncePads: [
    { id: 'bp30_c', x: 44, y: 10, power: -520 },
    { id: 'bp30_d', x: 58, y: 10, power: -520 }
  ],
  ...
  // Портал изначально на полу x=59, y=10. При приближении улетает наверх на постамент y=7!
  portal: {
    x: 59,
    y: 10,
    targets: [
