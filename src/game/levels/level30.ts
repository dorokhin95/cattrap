...
export const level30: LevelData = {
  ...
  solidTiles: [
    ...
    // Ступени и постамент для портала в Section D (x=55..62)
    // Ступенька 1 (y=10): x=55..56
    { x: 55, y: 10 }, { x: 56, y: 10 },
    // Ступенька 2 (y=9): x=57..58
    { x: 57, y: 9 }, { x: 58, y: 9 }, { x: 57, y: 10 }, { x: 58, y: 10 },
    // Верхняя площадка с порталом (y=8): x=59..62
    ...Array.from({ length: 4 }, (_, i) => ({ x: 59 + i, y: 8 })),
    ...Array.from({ length: 4 }, (_, i) => ({ x: 59 + i, y: 9 })),
    ...Array.from({ length: 4 }, (_, i) => ({ x: 59 + i, y: 10 })),
  ...
  // Батут на правом крае реверс-секции
  bouncePads: [
  ...
  // Портал изначально на полу x=59, y=10. При приближении улетает наверх на постамент x=60, y=7!
  portal: {
    x: 59,
    y: 10,
    targets: [
